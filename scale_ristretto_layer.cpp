...
ScaleRistrettoLayer<Dtype>::ScaleRistrettoLayer(
    const LayerParameter &param) : ScaleLayer<Dtype>(param), BaseRistrettoLayer<Dtype>()
{
...
template <typename Dtype>
void ScaleRistrettoLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
    ScaleParameter param = this->layer_param_.scale_param();
    ...
    if (param.bias_term()) {
        this->weights_quantized_[1].reset(new Blob<Dtype>(scale_shape));
    }
    */
}
    ...
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
    ...
    if (param.bias_term()) {
        caffe_copy(this->blobs_[1]->count(), this->blobs_[1]->cpu_data(),         this->weights_quantized_[1]->mutable_cpu_data());
    }
    ...
    */
    ...
    this->QuantizeWeights_cpu(this->blobs_, rounding, this->bias_layer_);
    ...
    if (bottom[0] == top[0])
    {
        ...
        caffe_copy(bottom[0]->count(), bottom[0]->cpu_data(), this->temp_.mutable_cpu_data());
    }
...
template <typename Dtype>
void ScaleRistrettoLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down, const vector<Blob<Dtype> *> &bottom)
{
    if (this->bias_layer_ && this->param_propagate_down_[this->param_propagate_down_.size() - 1])
    {
        ...
        (scale_param && this->param_propagate_down_[0]))
    {
        ...
        Dtype *product = (is_eltwise ? scale->mutable_cpu_diff() : (in_place ? this->temp_.mutable_cpu_data() : bottom[0]->mutable_cpu_diff()));
        ...
        if (!is_eltwise)
        {
            ...
            {
                const Dtype *sum_mult = this->sum_multiplier_.cpu_data();
                sum_result = (this->outer_dim_ == 1) ? scale->mutable_cpu_diff() : this->sum_result_.mutable_cpu_data();
                caffe_cpu_gemv(CblasNoTrans, this->sum_result_.count(), this->inner_dim_, Dtype(1), product, sum_mult, Dtype(0), sum_result);
            }
            if (this->outer_dim_ != 1)
            {
                ...
                {
                    caffe_cpu_gemv(CblasTrans, this->outer_dim_, this->scale_dim_, Dtype(1), sum_result, sum_mult, Dtype(scale_param), scale_diff);
                }
            }
        }
    }
