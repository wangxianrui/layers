    ...
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
    ...
    // Trim weights
    /*
    caffe_copy(this->blobs_[0]->count(), this->blobs_[0]->gpu_data(),       this->weights_quantized_[0]->mutable_gpu_data());
    if (param.bias_term()) {
        caffe_copy(this->blobs_[1]->count(), this->blobs_[1]->gpu_data(),         this->weights_quantized_[1]->mutable_gpu_data());
    }
    ...
    */
    ...
    this->QuantizeWeights_gpu(this->blobs_, rounding, this->bias_layer_);
    ...
    if (bottom[0] == top[0])
    {
        ...
        caffe_copy(bottom[0]->count(), bottom[0]->gpu_data(), this->temp_.mutable_gpu_data());
    }
...
template <typename Dtype>
void ScaleRistrettoLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down, const vector<Blob<Dtype> *> &bottom)
{
    if (this->bias_layer_ &&
        ...
        (scale_param && this->param_propagate_down_[0]))
    {
        ...
        Dtype *product = (is_eltwise ? scale->mutable_gpu_diff() : (in_place ? this->temp_.mutable_gpu_data() : bottom[0]->mutable_gpu_diff()));
        ...
        if (!is_eltwise)
        {
            ...
            {
                const Dtype *sum_mult = this->sum_multiplier_.gpu_data();
                sum_result = (this->outer_dim_ == 1) ? scale->mutable_gpu_diff() : this->sum_result_.mutable_gpu_data();
                caffe_gpu_gemv(CblasNoTrans, this->sum_result_.count(), this->inner_dim_, Dtype(1), product, sum_mult, Dtype(0), sum_result);
            }
            if (this->outer_dim_ != 1)
            {
                ...
                else
                {
                    ...
                    caffe_gpu_gemv(CblasTrans, this->outer_dim_, this->scale_dim_, Dtype(1), sum_result, sum_mult, Dtype(scale_param), scale_diff);
                }
            }
        }
    }
